...
export default function Board() {
    ...
    async function setHandCardsPlayer(cards, setHandCardsFunction) {
        const handCardImages = await Promise.all(cards.map(card =>
            importGameCard(card.color, card.exit, card.iniciative).then(module => ({ ...card, image: module.default }))
        ));
        setHandCardsFunction(handCardImages);
    }
    ...
    useEffect(() => {
        async function fetchGameCards() {
            try {
                ...
                fetch(`/api/v1/games/fivecards/${dataGamePlayer[0].id}`, {
                    headers: {
                        'Authorization': `Bearer ${jwt}`
                    }
                });
                fetch(`/api/v1/games/fivecards/${dataGamePlayer[1].id}`, {
                    headers: {
                        'Authorization': `Bearer ${jwt}`
                    }
                });
                ...
                const handCardsPlayer1 = data.filter(card => card.color === dataGamePlayer[0].color && card.card_Status === "IN_HAND");
                const handCardsPlayer2 = data.filter(card => card.color === dataGamePlayer[1].color && card.card_Status === "IN_HAND");

                await setHandCardsPlayer(handCardsPlayer1, setHandCardsPlayer1);
                await setHandCardsPlayer(handCardsPlayer2, setHandCardsPlayer2);
